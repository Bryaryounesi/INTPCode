...
# ============================================================
'''
این چیت‌شیت شامل تمام مباحث پردازش تصویر با اپن سیوی است:
خواندن و ذخیره تصویر، تغییرات هندسی، فیلترها، آستانه‌گذاری،
لبه و کانتور، مورفولوژی، بهبود کنتراست، ابزارهای کمکی،
و مباحث پروژه‌محور: طراحی Pipeline، کدنویسی تابع‌محور،
فایل config.py، ساختار پروژه استاندارد و انتشار در GitHub.
...
لزوم یکسان بودن کانال های رنگی در تصاویر مورد مقایسه در hstack و vstack
...
masked_bgr = cv2.cvtColor(masked, cv2.COLOR_GRAY2BGR)
...
blockSize: اندازه ناحیه بررسی — یک عدد فرد
           والیوهای رایج: 11 (جزئیات بیشتر) | 21 (نرم‌تر، پیشنهادی برای متون) | 39 (برای اشیاء بزرگ با پس‌زمینه ساده)
C: ثابت تصحیح — از میانگین محلی کم می‌شود
   ...
   منفی (مثلاً -1) = برای اشیاء طرح‌دار با پس‌زمینه سفید، مرز بیرونی بسته‌تر
...
غربالگری کانتورهای بسته بزرگ با آستانه مساحت:
min_areas = 50000
...
نکته: این روش برای حذف کانتورهای ریز و کاذب در پروژه‌های شمارش اشیاء
بسیار مفید است. آستانه min_areas باید به‌صورت تجربی و با آزمون و خطا
روی چند تصویر نمونه تنظیم شود تا تعداد کانتورهای باقی‌مانده با تعداد
واقعی اشیاء برابر شود.
...
نوشتن متن روی تصویر:
cv2.putText(img, "text", (x, y), cv2.FONT_HERSHEY_COMPLEX, 2, (0, 0, 255), 3)

پارامترها:
img: تصویر مقصد
text: رشته متنی
(x, y): مختصات شروع متن (گوشه پایین-چپ متن)
fontFace: نوع فونت | cv2.FONT_HERSHEY_COMPLEX یا FONT_HERSHEY_SIMPLEX
fontScale: اندازه فونت | 1 تا 3 رایج
color: رنگ متن (B, G, R)
thickness: ضخامت خط متن | 2 یا 3 رایج
...
Erosion (فرسایش):
eroded = cv2.erode(binary/gray, kernel, iterations=1)
...
Dilation (گسترش):
dilated = cv2.dilate(binary/gray, kernel, iterations=1)
...
# بهترین پایپلاین مورفولوژی
# این پایپلاین، فعلا در مینی پروژه مربوطه، تابع محور نشده  پس نمیتوان از آن به صورت آماده و با ایمپورت استفاده کرد
...
CLAHE شرطی برای تصاویر کم‌کنتراست:
if np.std(gray) < 50:
...
نکته: انحراف معیار (std) معیار خوبی برای سنجش کنتراست کلی تصویر است.
std < 50 یعنی تصویر کم‌کنتراست است و CLAHE می‌تواند کمک کند.
std > 50 یعنی تصویر کنتراست کافی دارد و CLAHE لازم نیست.
...
پایپلاین ۶: شمارش اشیاء ساده (پروژه هفته ۷)
    Read Image >> Grayscale >> Adaptive Threshold >> Opening >> Dilation >>
    Find Contours >> Filter by Area >> Draw Boxes >> Save Result
...
ساختار پروژه شمارش اشیاء (هفته ۷):
Simple_Object_Counting/
├── data/                  ← تصاویر ورودی (.jpg)
├── output/                ← تصاویر پردازش‌شده
├── src/
│   ├── config.py          ← تنظیمات ثابت و مسیرها
│   └── main.py            ← اسکریپت اصلی با توابع پایپلاین
├── README.md              ← مستندسازی پروژه
└── soc_requirements.txt   ← کتابخانه‌های مورد نیاز

جریان کلی:
data (ورودی‌ها) → src (پردازش) → outputs (نتایج)

# ============================================================
# ۲۱. کدنویسی تابع‌محور (Function-Based Coding)
# ============================================================

مفهوم:
تابع‌محور نوشتن یعنی پایپلاین رو به چند بلوک مستقل و نام‌دار تقسیم کنید
که هرکدوم یک مسئولیت مشخص دارن، به‌جای اینکه همه مراحل (خواندن، پردازش،
تحلیل، نمایش) پشت‌سر هم و در یک بلوک بزرگ نوشته بشن.

چرا این کار مهمه، مخصوصاً در پروژه‌های OpenCV:
- می‌تونید یک مرحله رو مستقل تست کنید بدون اجرای کل پایپلاین
- اگه بخواید همون منطق رو روی چند رنگ/چند دیتاست/چند شرط تکرار کنید،
  به‌جای کپی-پیست کد، فقط تابع رو صدا می‌زنید
- خوندن و دیباگ کردن کد آسون‌تر می‌شه

نکته مهم: همیشه نیازی به تابع‌محور کردن نیست. اگه پایپلاین کوتاهه،
فقط یک‌بار اجرا می‌شه، و پارامترها کمن، همون حلقه ساده کاملاً کافیه.

الگوی استاندارد تقسیم‌بندی:
# الگوی تابع محور کردن توابع و کد پروژه
# سطوح تابع محور

سطح 1:
- بخش تنظیمات (Config): مقادیر ثابت مثل مسیر پوشه‌ها، سایز کرنل‌ها،
  آستانه‌ها؛ بیرون از هر تابع یا در فایل config.py

سطح 2:
- تابع پیش‌پردازش (Preprocess): از تصویر خام تا تصویر آماده تحلیل
  (Gray، Threshold، Morphology)

سطح 3:
- تابع تحلیل/الگوریتم اصلی (Analyze/Detect): منطق اصلی پروژه
  (پیدا کردن Contour، فیلتر کردن، شمارش)

سطح 4:
- تابع ساخت خروجی/نمایش (Visualize/Build Output): ساخت تصویر نهایی
  برای نمایش یا ذخیره

سطح 5:
سطح I/O
فقط حاوی دو تابع ایمرید و ایمرایت است

سطح 6:
- بلوک اصلی اجرا (Main Loop / Orchestrator): فقط توابع بالا رو به ترتیب
  صدا می‌زنه؛ خودش منطق پردازشی نداره

قواعد نام‌گذاری و پارامتر توابع:

سینتکس کلی:
def نام_تابع(ورودی‌های لازم) -> خروجی:
    # منطق پردازشی
    return نتیجه

قواعد رایج:
- اسم تابع فعل + هدف باشه (مثل get_valid_contours، build_comparison)
- تابع فقط چیزی رو به‌عنوان پارامتر بگیره که واقعاً بهش نیاز داره
- مقادیر ثابت (کرنل، آستانه) به‌عنوان پارامتر پیش‌فرض یا از بیرون (Config)
  به تابع داده بشن، نه Hardcode داخل تابع
- تابع باید return کنه، نه فقط print یا imshow؛ نمایش/چاپ باید در بلوک
  اصلی انجام بشه، نه داخل تابع پردازشی

اشتباه رایج:
تابعی بسازید که هم پردازش می‌کنه هم imshow صدا می‌زنه هم print می‌کنه؛
این باعث می‌شه تابع قابل‌استفاده مجدد نباشه.

بهترین شیوه:
هر تابع رو طوری بنویسید که بتونید بدون اجرای کل فایل، جداگانه تستش کنید.

کد نمونه کامل (الگوی عمومی):
# الگوی تابع محور نویسی
...
# ---------------- Analyze ----------------
def find_objects(cleaned):
    contours, _ = cv2.findContours(cleaned, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    valid_contours = [c for c in contours if cv2.contourArea(c) > th_area]
    return valid_contours

# ---------------- Visualize ----------------
def draw_result(img, contours):
    result = img.copy()
...
# ---------------- Main ----------------
def object_detect(img):  #تابع اصلی پایپلاین(ریترن نتیجه کل پروژه)
    dilated = preprocess(img)
    best_contours = find_contours(dilated)
    result = draw_and_count_contours(img, best_contours)
    return result
----------------(main loop)لوپ اجرای دسته ای توابع پایپلاین برای دیباگ راحت تر بعدی----------
for i in paths:
    img = read_img(i)
    if img is None:
        continue
    cleaned = preprocess(img)
    objects = find_objects(cleaned)
    result = draw_result(img, objects)
    name = Path(i).stem
    p(f"{name} → {len(objects)} اشیاء تشخیص داده شد")
    save_img(outputs / f"{name}_result.jpg", result)
# --------------------------------------------------------
# ----------------------------------------------------------
نحوه ایمپورت بعدی تابع پایپلاین بر روی داتاست های دیگر در فایل های دیگر

from main import object_detect
new_paths = [...]
for i in paths:
    img = cv2.imread(i)
    result = object_detect(img)
    cv2.imshow("win",img) تصاویر جدید تشخیص اشیا میشوند
# ============================================================
# ۲۲. فایل config.py (جداسازی تنظیمات)
# ============================================================
# فایل کانفیگ جدا شده از ماین

مفهوم:
config.py یک فایل جداگانه است که فقط مقادیر ثابت و قابل‌تنظیم پروژه
(نه منطق پردازشی) رو نگه می‌داره.

چرا این کار مهمه:
- جدا کردن «تنظیمات» از «منطق»: وقتی بخواید مسیر پوشه دیتا رو عوض کنید
  یا آستانه مساحت رو تغییر بدید، فقط یک فایل کوچیک رو باز می‌کنید
- قابلیت استفاده مجدد: اگه بعداً یک اسکریپت دوم بنویسید، هر دو می‌تونن
  همون config.py رو ایمپورت کنن
- آماده‌سازی برای پروژه‌های بزرگ‌تر (PyTorch): تعداد پارامترهای قابل‌تنظیم
  خیلی بیشتر می‌شه؛ این جداسازی پایه‌ی همون ساختاریه که بعداً لازم می‌شه

نمونه فایل config.py:
...
# -------------------------------------------------
نحوه استفاده در main.py:

import cv2
import numpy as np
from pathlib import Path
# نحوه ایمپورت کردن فایل کانفیگ در پروژه :
# ایمپورت کردن متغیرهای کانفیگ در پروژه
...
# preprocess, find_objects, draw_boxes همون قبلی، بدون تغییر
# ...
...
for i in paths:
    ...
    cv2.imwrite(str(outputs / f"{title}.jpg"), boxes)

# ============================================================
# ۲۳. مستندسازی پروژه (README.md)
# ============================================================

بخش‌های ضروری یک README کامل:
1. معرفی پروژه (Project Introduction): توضیح مختصر هدف و کاربرد
2. دیتاست (Dataset): توضیح منبع و ویژگی‌های داده
3. پایپلاین (Pipeline): نمایش مراحل به‌صورت زنجیره
4. ساختار کد (Code Structure): جدول توابع و مسئولیت‌ها
5. ساختار پوشه‌ها (Project Structure): نمایش درختی فایل‌ها
6. نیازمندی‌ها (Requirements): دستور نصب کتابخانه‌ها
7. روش اجرا (Usage): دستور اجرای پروژه
8. جزئیات فنی (Pipeline Details): توضیح انتخاب‌ها و پارامترها
9. تحلیل نتایج (Analysis & Findings): آمار موفقیت، مشکلات، خطاها
10. محدودیت‌ها (Known Limitations): محدودیت‌های شناخته‌شده
11. لایسنس (License): نوع مجوز استفاده

نکته: README باید به زبانی نوشته شود که مخاطب هدف (حتی خود شما
بعد از چند ماه) بتواند به‌راحتی پروژه را درک و اجرا کند.
دوزبانه بودن (فارسی/انگلیسی) برای پروژه‌های آموزشی توصیه می‌شود.
